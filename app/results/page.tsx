...
const page = () => {
  ...
  const uniqueAlbumImages = Array.from(new Set(devData.songs.map(song => song.image))).slice(0, 4);
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="border inline-flex justify-center  ">
        {uniqueAlbumImages.map((img, index) => (
          <img key={index} src={img} alt="Album Cover" className="shadow-lg aspect-square object-contain" />
        ))}
      </div>

      {/* Playlist Description */}
      <div className="mb-8 text-center">
        <h2 className="text-3xl font-bold">Your Personalized Playlist</h2>
        <p className="text-gray-600 mt-2">Top hits based on your selected mood or activity.</p>
      </div>

      {/* Songs List */}
      <div className="space-y-4">
        {songsToShow.map((song, idx) => (
          <div key={idx} className="flex items-center gap-4 p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition">
            <img src={song.image} alt={song.album} className="w-14 h-14 rounded" />
            <div className="flex-1">
              <p className="font-semibold text-lg">{song.name}</p>
              <p className="text-sm text-gray-500">{song.artist} • {song.album}</p>
            </div>
            <a href={`https://open.spotify.com/track/${song.uri.split(':').pop()}`} target="_blank" rel="noopener noreferrer" className="text-green-600 hover:underline text-sm">Play</a>
          </div>
        ))}
      </div>
